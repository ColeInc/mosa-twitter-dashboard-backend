...
const getAccessToken = (req: Request, res: Response) => {
    try {
        ...
        console.log(error);
        res.status(403).send("Invalid verifier or access token provided!");
    }
};
...
const refreshToken = async (req: Request, res: Response) => {
    // all refresh logic is now offloaded into authentication middleware instead of in this controller.
    const { id, name, username } = res.locals.jwt;
    res.status(200).json({ userData: { id, name, username }, success: true });
};
